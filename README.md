...
상세 기능 및 데이터 플랫폼 요구사항은 프로젝트 상위의 `requirements.md`에서 관리합니다.
...
`main-app.js`, `app-pages-internals.js` 또는 청크 파일 404가 발생하면 실행 중인 개발 서버를 모두
종료한 뒤 캐시를 정리해서 다시 실행합니다.

```bash
npm run dev:clean
```

터미널에 표시된 정확한 주소로 접속하고 브라우저에서 강력 새로고침합니다.
